...
def create_import_from_path(path):
  segments = path.split("/")
  # hack, remove lib
  segments.pop()
  segments.pop()
...
def package_config_action(ctx, dart_ctx, output):
  """Creates an action that generates a Dart package config file.
  ...
  content = '{"configVersion": 2,"generator": "Bazel","packages": ['
  comma = False
  for dc in dart_ctxs:
    ...
    if (comma):
      content += ",\n"
    comma = True
    package_name = create_import_from_path(dc.lib_root)
    package_path = "file:///Users/jonahwilliams/Documents/rules_dart/" + dc.lib_root
    content += '{"name": "%s", "rootUri": "%s", "languageVersion": "%s", "packageUri": "%s"}' % (package_name, package_path, "2.12", "")
